...
from GaussianPolicyGradient import Gaussian_Policy_Gradient
import numpy as np
...
"""############################################################################################################"""
""" parameters shared with main running file and must agree, this is only here because the code need to be polished more"""
...
seed=2
#seed for gaussian RL initialization
g_seed = 2
#seed for the manufacturing system
np_seed = 1
...
""" a parameter pertinent to this piece of code"""    
...
"""
Given the current Gaussian actor and omega, determine the next continuous and discrete/remainder actions by finding 
(1) A^c(theta)=energy distributed in [solar, wind, generator]
(2) with probability probability_randomaction: A^{*,d}_{t+1}=randomly sampled action
    with probability 1-probability_randomaction: A^{*,d}_{t+1}=argmin_{A^d}Q(S_{t+1}, A^d, A^c(theta), A^r(A^d, A^c(theta)); omega)
(3) A^{*,r}_{t+1}=A^r(A^{*,d}_{t+1}, A^c(theta))
"""
def NextAction_OnGaussianPolicySimulation(next_machine_states, next_buffer_states, next_workingstatus, next_SOC, t, my_critic, my_continuos_actor, probability_randomaction):
    #build an auxiliarysystem with next system and grid states#
    AuxiliarySystem=ManufacturingSystem(machine_states=next_machine_states,
                                        machine_control_actions=["K" for _ in range(number_machines)],
                                        buffer_states=next_buffer_states,
    ...
    energy_generated_wind=AuxiliarySystem.grid.energy_generated_wind()
    energy_generated_generator=AuxiliarySystem.grid.energy_generated_generator()
    #obtain theta from the gaussian actor. The actor is thus determining the energy distribution over the system
    state_vector = np.reshape(np.array([energy_generated_solar,energy_generated_wind,energy_generated_generator],dtype=np.float32), (3,1))
    gaussian_action = projection(my_continuos_actor.takeAction(state_vector).numpy())
    theta = gaussian_action
    #under the current theta, calculate the continuous actions A^c(theta)=energy distributed in [solar, wind, generator]#
    next_actions_solar=[energy_generated_solar*theta[1-1], energy_generated_solar*theta[2-1], energy_generated_solar*(1-theta[1-1]-theta[2-1])]
    next_actions_wind=[energy_generated_wind*theta[3-1], energy_generated_wind*theta[4-1], energy_generated_wind*(1-theta[3-1]-theta[4-1])]
    next_actions_generator=[energy_generated_generator*theta[5-1], energy_generated_generator*theta[6-1], energy_generated_generator*(1-theta[5-1]-theta[6-1])]
    ...
    return next_machine_actions, next_actions_solar, next_actions_wind, next_actions_generator, next_microgrid_actions_adjustingstatus, next_microgrid_actions_purchased, next_microgrid_actions_discharged, gaussian_action
...
def Reinforcement_Learning_Training(System_init, #the initial system
    ...
    plt.savefig(f'weightdifference{ID}.png')
    plt.show()   
    
    #record the training process onto a file train_output.txt    
    troutput = open(f'train_output{ID}.txt', 'w')
    ...
    print("\nthe seed for choosing initial weight and bias parameter is given by: ", rand_seed, file=troutput)
...
"""
Gaussian Reinforcement Learning Algorithm: On-policy TD control combined with actor-critique
"""
def Guassian_RL_Training(System_init, #the initial system
                         lr_theta_init, #the initial learning rate in the theta iteration
                         lr_omega_init, #the initial learning rate in the omega iteration
                         number_iteration, #the total number of training iterations
                         rand_seed = g_seed,
    ...
    lr_tht=lr_theta_init
    lr_omg=lr_omega_init
    #actur that will decide on the energy distribution theta in R^9
    gaussian_actor = Gaussian_Policy_Gradient(seed = rand_seed, action_dim = 6)
    #gaussian_thetainit = [gaussian_actor.theta_mu.numpy(),gaussian_actor.theta_sigma.numpy()]
    theta_init = None
    ...
    for t in range(number_iteration):
        ...
        #get derivatives of action wrt gaussian action
        upd_tht=update_theta(System)
        A_c_grad_gaussAction = upd_tht.A_c_gradient_theta()
        ...
        #determine A^{*,d}_{t+1}=argmin_{A^d}Q(S_{t+1}, A^d, A^c(theta), A^r(A^d, A^c(theta)); omega)
        #determine A^{*,r}_{t+1}=A^r(A^{*,d}_{t+1}, A^c(theta))
        next_machine_actions, next_actions_solar, next_actions_wind, next_actions_generator, next_microgrid_actions_adjustingstatus, next_microgrid_actions_purchased, next_microgrid_actions_discharged, gaussian_action = NextAction_OnGaussianPolicySimulation(next_machine_states, 
                                                                                                                                                                                                                                         next_buffer_states, 
                                                                                                                                                                                                                                         next_workingstatus, 
                                                                                                                                                                                                                                         next_SOC,
                                                                                                                                                                                                                                         t, 
                                                                                                                                                                                                                                         ...
                                                                                                                                                                                                                                         gaussian_actor,
                                                                                                                                                                                                                                         probability_randomaction=p_choose_random_action)
        #update the theta using gaussian policy gradient#
        #policy_grad=upd_tht.deterministic_policygradient(upd_tht.A_c_gradient_theta(), av.Q_grad_A_c())
        theta = gaussian_action
        if theta_init == None: theta_init=[gaussian_actor.theta_mu.numpy(),gaussian_actor.theta_sigma.numpy()]
        gaussian_actor.update_weights(gaussian_action,A_c_grad_gaussAction,av.Q_grad_A_c(),lr_tht)
    ...
    #plot the theta dynamics#
    plt.savefig(f'theta_gaussian{ID}.png')
    plt.show()  
    ...
    plt.xlabel('iteration')
    plt.ylabel('action-value-function')
    plt.savefig(f'Q_gaussian{ID}.png')
    plt.show() 
    ...
    plt.xlabel('iteration')
    plt.ylabel('temporal difference function')
    plt.savefig(f'TD_gaussian{ID}.png')
    plt.show()   
    ...
    plt.xlabel('iteration')
    plt.ylabel('Sum of rewards during episode ($)')
    plt.savefig(f'rewards_gaussian{ID}.png')
    plt.show()       
    ...
    plt.xlabel('iteration')
    plt.ylabel('L2 norm of the difference in the weights')
    plt.savefig(f'weightdifference_gaussian{ID}.png')
    plt.show()   
    
    #record the training process onto a file train_output.txt    
    troutput = open(f'train_output_gaussian{ID}.txt', 'w')
    print("************************ Reinforcement Learning Training at "+str(number_iteration)+" steps ************************", file=troutput)
    theta = [gaussian_actor.theta_mu.numpy(),gaussian_actor.theta_sigma.numpy()]
    #print the initial theta and optimal theta after training
    print("\nthe initial theta is given by: ", theta_init, file=troutput)
    print("\nthe optimal theta after training is given by: ", theta, file=troutput)
    #print the L2 difference of the theta between initial and final
    ...
    return gaussian_actor, omega, my_critic, theta_init
...
def Reinforcement_Learning_Testing(System_init, #the initial point of the system dynamics
    ...
    testoutput = open(f'test_output{ID}.txt', 'w') 
...
Reinforcement Learning Algorithm: Off policy TD control combined with actor-critique
Algorithm 1 in the paper
The Testing Process of the Reinforcement Learning Algorithm under Gaussian continuous policy
Output at a given horizon the system dynamics under the optimal action selected by Reinforcement Learning training
select the best action-value function at each iteration
"""
def RL_Gaussian_Testing(System_init, #the initial point of the system dynamics
                                   gaussian_actor,   #the trained actor  
                                   initialWeights, #initial actor weights
                                   omegaoptimal,    #the optimal parameter of critic obtained after training
                                   my_critic_optimal, #the optimal critic obtained after training
                                   number_iteration, #the total number of testing iterations
                                   unit_reward_production, #the unit reward of production used to calculate target output
                                   ID = ""):
    thetainit = initialWeights
    thetaoptimal = [gaussian_actor.theta_mu.numpy(),gaussian_actor.theta_sigma.numpy()]
    #output the result to a file test_output.txt#
    testoutput = open(f'test_output_gaussian{ID}.txt', 'w') 

    print("************************* Optimal System with optimal policy *************************", file=testoutput)
    print("***Run the system on optimal policy at a time horizon=", number_iteration,"***", file=testoutput)
    print("\n", file=testoutput)
    print("initial actor weights=", thetainit, file=testoutput)
    print("optimal actor weights=", thetaoptimal, file=testoutput) #outputs actor weights
    print("optimal parameter for the neural-network integrated action-value function=", omegaoptimal, file=testoutput)
    print("\n", file=testoutput)
    #run the MDP under optimal theta and optimal omega#
    #at every step search among all discrete actions to find A^d_*=argmin_{A^d}Q(A^d, A^c(thetaoptimal), A^r(A^d, A^c(thetaoptimal)))#
    ...
    for t in range(number_iteration):
        ...
        #determine the next discrete actions by finding A^{*,d}_{t+1}=argmin_{A^d}Q(S_{t+1}, A^d, A^c(thetaoptimal), A^r(A^d, A^c(thetaoptimal)); omegaoptimal)#
        #determine the next remainder actions A^{*,r}_{t+1}=A^r(A^{*,d}_{t+1}, A^c(thetaoptimal))
        optimal_next_machine_actions, optimal_next_actions_solar, optimal_next_actions_wind, optimal_next_actions_generator, optimal_next_microgrid_actions_adjustingstatus, optimal_next_microgrid_actions_purchased, optimal_next_microgrid_actions_discharged, gaussian_action = NextAction_OnGaussianPolicySimulation(next_machine_states, 
                                                                                                                                                                                                                                                                                                            next_buffer_states, 
                                                                                                                                                                                                                                                                                                            next_workingstatus, 
                                                                                                                                                                                                                                                                                                            next_SOC,
                                                                                                                                                                                                                                                                                                            t, 
...
def Benchmark_RandomAction_Testing(System_init, #the inital point of running the system dynamics
    ...
    bmoutput = open(f'benchmark_output{ID}.txt', 'w')
...
if __name__ == "__main__":
    ...
    training_number_iteration_gauss = training_number_iteration
    testing_number_iteration=4
    ...
    gaussian_actor, gaussian_omega, gaussian_critic, gaussian_thetainit = Guassian_RL_Training(System, 
                                                                            lr_theta_initial, 
                                                                            lr_omega_initial, 
                                                                            training_number_iteration_gauss
                                                                            )
    ...
    # As a benchmark, run the system with gaussian action
    System=SystemInitialize(initial_machine_states, initial_machine_actions, initial_buffer_states)
    ...
    #plot the total cost#
    plt.figure(figsize = (14,10))
    plt.plot([value*10000 for value in totalcostlist_optimal], '-', color='r',label = "det. policy")
    plt.plot([value*10000 for value in totalcostlist_optimal_gaussian], '-.', color='m', label = "gauss. policy")
    plt.plot([value*10000 for value in totalcostlist_benchmark], '--', color='b',label = "rand. policy")
    plt.xlabel('iteration')
    ...
    plt.plot([value*10000 for value in totalthroughputlist_benchmark], '--', color='b',label = "rand. policy")
    plt.xlabel('iteration')
    ...
    plt.plot([value/unit_reward_production for value in totalthroughputlist_benchmark], '--', color='b',label = "rand. policy")
    plt.xlabel('iteration')
    ...
    plt.plot([value*10000 for value in totalenergydemandlist_benchmark], '--', color='b',label = "rand. policy")
    plt.xlabel('iteration')
