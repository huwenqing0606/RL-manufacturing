...
class Microgrid(object):
    ...
    def transition(self):
        if self.actions_adjustingstatus[1]==1:
            self.workingstatus[1]=1
        else:
            self.workingstatus[1]=0
        #determining the next decision epoch working status of solar PV, 1=working, 0=not working#
        if self.actions_adjustingstatus[2]==0 or self.windspeed>cutin_windspeed or self.windspeed<cutoff_windspeed:
            self.workingstatus[2]=0
        else: 
            if self.actions_adjustingstatus[2]==1 and self.windspeed<=cutin_windspeed and self.windspeed>=cutoff_windspeed:
                self.workingstatus[2]=1
        #determining the next decision epoch working status of wind turbine, 1=working, 0=not working#        
        if self.actions_adjustingstatus[3]==1:
            self.workingstatus[3]=1
        else:
            self.workingstatus[3]=0
        #determining the next decision epoch working status of generator, 1=working, 0=not working#
        self.SOC=self.SOC+(self.actions_solar[2]+self.actions_wind[2]+self.actions_generator[2]+self.actions_purchased[2])*charging_discharging_efficiency-self.actions_discharged/charging_discharging_efficiency
        #update the SOC, state of charge of the battery system#
    
    def EnergyConsumption(self):
        #returns the energy consumption from the grid#
        return -(self.actions_solar[1]+self.actions_wind[1]+self.actions_generator[1]+self.actions_discharged)
    ...
    def OperationalCost(self):
        ...
        if self.workingstatus[2]==1 and self.windspeed<rated_windspeed and self.windspeed>=cutin_windspeed:
            ...
            if self.workingstatus[2]==1 and self.windspeed<cutoff_windspeed and self.windspeed>=rated_windspeed:
        ...
        operational_cost+=(self.actions_discharged+self.actions_solar[2]+self.actions_wind[2]+self.actions_generator[2])*Delta_t*unit_operational_cost_battery/(2*capacity_battery_storage*(SOC_max-SOC_min))
    ...
    def SoldBackReward(self):
        ...
        return (self.actions_solar[3]+self.actions_wind[3]+self.actions_generator[3])*unit_reward_soldbackenergy
